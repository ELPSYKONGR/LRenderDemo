...
std::shared_ptr<Texture2D> EffectResource::CreateTexture(std::string name, const std::filesystem::path& path,
                                                         bool forceSrgb)
{
    if (name.empty())
    {
...
std::shared_ptr<Texture2D> EffectResource::AddTexture(std::string name, std::shared_ptr<Texture2D> texture)
{
...
RenderTarget& EffectResource::CreateRenderTarget(std::string name, std::uint32_t width, std::uint32_t height)
{
    if (name.empty())
    {
...
ID3D11ShaderResourceView* EffectResource::GetShaderResource(std::string_view name) const noexcept
{
...
EffectResource& IRenderEffect::Resources() noexcept
{
    return m_resources;
}

const EffectResource& IRenderEffect::Resources() const noexcept
{
    return m_resources;
}
