/**
 * @file Fullscreen triangle vertex shader for post processing.
 * @depends common.hlsli
 */
...
PixelInput VSMain(uint vertexId : SV_VertexID)
{
    ...
    const float2 coordinates[3] = {float2(0.0F, 1.0F), float2(0.0F, -1.0F), float2(2.0F, 1.0F)};
    PixelInput output;
