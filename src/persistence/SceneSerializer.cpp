...
std::filesystem::path ResolveResourcePath(
    const std::filesystem::path& value, const std::filesystem::path& sceneDirectory) {
    return value.is_absolute() ? value.lexically_normal()
                               : (sceneDirectory / value).lexically_normal();
}

std::string StoreResourcePath(
    const std::filesystem::path& value, const std::filesystem::path& sceneDirectory) {
    if (value.empty()) {
        return {};
    }
    const std::filesystem::path absolute = value.is_absolute()
        ? value.lexically_normal() : std::filesystem::absolute(value).lexically_normal();
...
Json Vector(const Vector2& value) { return Json::array({value.x, value.y}); }
Json Vector(const Vector3& value) { return Json::array({value.x, value.y, value.z}); }
Json Vector(const Color& value) { return Json::array({value.x, value.y, value.z, value.w}); }

template <std::size_t Size>
std::array<float, Size> ReadVector(const Json& value, const char* field) {
...
const char* DisplayModeName(SurfaceDisplayMode value) {
    switch (value) {
    case SurfaceDisplayMode::LitTextured: return "lit-textured";
    case SurfaceDisplayMode::TextureOnly: return "texture-only";
    case SurfaceDisplayMode::LitUntextured: return "lit-untextured";
    }
...
const char* FilterName(MaterialFilter value) {
    switch (value) {
    case MaterialFilter::Point: return "point";
    case MaterialFilter::Linear: return "linear";
    case MaterialFilter::Anisotropic: return "anisotropic";
    }
...
const char* AddressName(MaterialAddressMode value) {
    switch (value) {
    case MaterialAddressMode::Wrap: return "wrap";
    case MaterialAddressMode::Clamp: return "clamp";
    case MaterialAddressMode::Mirror: return "mirror";
    }
    throw std::invalid_argument("Unknown material address mode");
}

template <typename Enum>
Enum ReadEnum(const std::string& value);

template <>
SurfaceDisplayMode ReadEnum(const std::string& value) {
    if (value == "lit-textured") return SurfaceDisplayMode::LitTextured;
    if (value == "texture-only") return SurfaceDisplayMode::TextureOnly;
    if (value == "lit-untextured") return SurfaceDisplayMode::LitUntextured;
    throw std::runtime_error("Unknown surface display mode: " + value);
}

template <>
MaterialFilter ReadEnum(const std::string& value) {
    if (value == "point") return MaterialFilter::Point;
    if (value == "linear") return MaterialFilter::Linear;
    if (value == "anisotropic") return MaterialFilter::Anisotropic;
    throw std::runtime_error("Unknown material filter: " + value);
}

template <>
MaterialAddressMode ReadEnum(const std::string& value) {
    if (value == "wrap") return MaterialAddressMode::Wrap;
    if (value == "clamp") return MaterialAddressMode::Clamp;
    if (value == "mirror") return MaterialAddressMode::Mirror;
    throw std::runtime_error("Unknown material address mode: " + value);
}

Json SerializeGeometry(
    const Entity& entity, const std::filesystem::path& sceneDirectory) {
    ...
    std::visit([&](const auto& value) {
        ...
        if constexpr (std::is_same_v<Parameters, CubeParameters>) {
            ...
            type = "sphere";
            parameters = {
                {"radius", value.radius}, {"slices", value.slices}, {"stacks", value.stacks}};
        } else {
            type = "plane";
            parameters = {
...
Json SerializeMaterial(
    const EntityMaterial& material, const std::filesystem::path& sceneDirectory) {
    return {
        {"baseColor", Vector(material.baseColor)},
        ...
        {"baseColorTexturePath",
         StoreResourcePath(material.baseColorTexturePath, sceneDirectory)},
...
SolidGeometry DeserializeSolid(const Json& geometry) {
    ...
    if (type == "sphere") {
        return SolidGeometry::Sphere({
            parameters.at("radius").get<float>(),
...
EntityMaterial DeserializeMaterial(
    const Json& value, const std::filesystem::path& sceneDirectory) {
    ...
    material.displayMode = ReadEnum<SurfaceDisplayMode>(
        value.at("displayMode").get<std::string>());
    ...
    material.addressMode = ReadEnum<MaterialAddressMode>(
        value.at("addressMode").get<std::string>());
    return material;
}
...
Entity DeserializeEntity(const Json& value, const std::filesystem::path& sceneDirectory) {
    ...
    const auto rotation = ReadVector<3>(
        transform.at("rotationDegrees"), "transform.rotationDegrees");
    ...
    if (kind == "solid") {
        ...
        entity.geometry = MeshGeometry{
            ResolveResourcePath(
                Utf8Path(geometry.at("assetPath").get<std::string>()), sceneDirectory),
...
void SceneSerializer::Save(const Scene& scene, const std::filesystem::path& path) {
    ...
    try {
        ...
        if (MoveFileExW(
                temporary.c_str(), absolutePath.c_str(),
...
Scene SceneSerializer::Load(const std::filesystem::path& path) {
    ...
    try {
        ...
        throw std::runtime_error(
            "Failed to load scene " + PathUtf8(path) + ": " + error.what());
    }
}
