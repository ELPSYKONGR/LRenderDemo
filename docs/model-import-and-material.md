...
```mermaid
graph LR
    Scene[场景实体和模型路径] --> Renderer[Dx11Renderer]
...
5. 在 `Lighting` 面板调整环境光、一盏方向光和四盏点光。
6. 观察 `Resources` 面板；重复导入同一路径时，缓存数量不应增加。
