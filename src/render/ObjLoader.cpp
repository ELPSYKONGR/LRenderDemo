...
struct VertexKeyHash {
    std::size_t operator()(const VertexKey& key) const noexcept {
        ...
        value ^= std::hash<int>{}(key.textureCoordinate) + 0x9e3779b9U +
                 (value << 6U) + (value >> 2U);
        return value;
    }
};
...
std::uint32_t AddVertex(
    PartBuilder& builder, const tinyobj::index_t& source,
    ...
    if (source.normal_index >= 0) {
        ...
        vertex.normal = {
            ...
            attributes.normals[normalOffset + 1U],
    ...
    if (source.texcoord_index >= 0) {
        ValidateIndex(
            source.texcoord_index, attributes.texcoords.size() / 2U, "texture coordinate");
...
Material LoadMaterial(
    ...
    material.name = source.name.empty() ? "OBJ material" : source.name;
    material.baseColorFactor = {
        source.diffuse[0], source.diffuse[1], source.diffuse[2], source.dissolve};
    material.specularColor = {
        source.specular[0], source.specular[1], source.specular[2], 1.0F};
    material.specularStrength = 1.0F;
    material.shininess = std::clamp(source.shininess, 1.0F, 256.0F);
    if (!source.diffuse_texname.empty()) {
        material.baseColorTexture = resources.LoadTexture(
            modelPath.parent_path() / Utf8RelativePath(source.diffuse_texname));
    }
    return material;
}

MeshAssetEntity LoadShape(
    const tinyobj::shape_t& shape, std::size_t shapeIndex,
    const tinyobj::attrib_t& attributes,
    ...
    for (std::size_t faceIndex = 0;
        ...
        const std::array<std::uint32_t, 3> face{
            AddVertex(builder, shape.mesh.indices[sourceIndex], attributes),
    ...
    entity.name = shape.name.empty()
        ? "OBJ Shape " + std::to_string(shapeIndex + 1U) : shape.name;
    for (PartBuilder& builder : builders) {
        ...
        entity.parts.push_back({
            std::make_unique<Mesh>(resources.Device(), builder.vertices, builder.indices),
...
std::shared_ptr<MeshAsset> ObjLoader::Import(
    ...
    if (!reader.ParseFromFile(mesh_import::PathUtf8(path), configuration)) {
        throw std::runtime_error(
            "Failed to parse OBJ " + mesh_import::PathUtf8(path) + ": " + reader.Error());
    }
    ...
    for (std::size_t index = 0; index < shapes.size(); ++index) {
        MeshAssetEntity entity = LoadShape(
            shapes[index], index, reader.GetAttrib(), reader.GetMaterials(),
            path, resources);
    ...
    if (asset->entities.empty()) {
        throw std::runtime_error("OBJ contains no supported triangle mesh: " +
                                 mesh_import::PathUtf8(path));
    }
    return asset;
}
