...
void TestSceneRoundTrip() {
    const std::filesystem::path directory =
        std::filesystem::current_path() / "scratch" / "tests";
    ...
    const auto& mesh = source.CreateMeshEntity(
        modelId, "assets/test-scenes/sample.obj", 3, "Imported");
    const lrender::EntityId meshId = mesh.id;
    const std::filesystem::path expectedMeshPath =
        std::filesystem::absolute(mesh.Mesh()->assetPath).lexically_normal();
    ...
    Require(loaded.Models()[0].entities.size() == 2,
            "Scene entity count should survive round trip");
    const lrender::Entity* loadedSphere = loaded.FindEntity(sphereId);
    Require(loadedSphere != nullptr && loadedSphere->IsSolid(),
            "Parameterized solid should survive round trip");
    const auto& parameters = std::get<lrender::SphereParameters>(
        loadedSphere->Solid()->Parameters());
    RequireNear(parameters.radius, 1.75F, "Sphere radius should survive round trip");
    Require(parameters.slices == 40 && parameters.stacks == 22,
    ...
    Require(loadedMesh != nullptr && loadedMesh->IsMesh(),
            "Mesh reference should survive round trip");
    Require(
        loadedMesh->Mesh()->assetPath == expectedMeshPath,
