...
void EditorLayer::DrawSolidGeometryEditor(
    Scene& scene, CommandHistory& history, Entity& entity) {
    SolidGeometry* solid = entity.Solid();
    if (solid == nullptr || !ImGui::CollapsingHeader(
            "Geometry", ImGuiTreeNodeFlags_DefaultOpen)) {
        return;
    }
    ...
    if (const auto* cube = std::get_if<CubeParameters>(&solid->Parameters())) {
        ...
        if (ImGui::DragFloat3(
                "Dimensions", &parameters.size.x, 0.02F, 0.001F, 10000.0F, "%.3f")) {
        ...
        if (ImGui::DragFloat(
                "Radius", &parameters.radius, 0.01F, 0.001F, 10000.0F, "%.3f")) {
...
void EditorLayer::TrackSolidEdit(
    Scene& scene, CommandHistory& history, Entity& entity,
