...
ColorProcessorEffect::ColorProcessorEffect(
    ...
    ThrowIfFailed(Device()->CreateVertexShader(
        ...
        m_vertexShader.GetAddressOf()), "Failed to create color processor vertex shader");
    ThrowIfFailed(Device()->CreatePixelShader(
        pixelShader->GetBufferPointer(), pixelShader->GetBufferSize(), nullptr,
        m_pixelShader.GetAddressOf()), "Failed to create color processor pixel shader");
}

void ColorProcessorEffect::Bind(
    const EffectFrameContext& frame, const EffectDrawContext&) {
