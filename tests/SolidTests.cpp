...
void TestParameterizedMeshCache()
{
    ...
    Require(rebuilt.IndexCount() == 12U * 6U * 6U, "Edited topology should rebuild the runtime mesh");

    const auto plane = lrender::SolidGeometry::Plane({{4.0F, 6.0F}, 2, 3});
    Require(cache.Resolve(9, plane).IndexCount() == 2U * 3U * 6U, "Subdivided plane index count is incorrect");
