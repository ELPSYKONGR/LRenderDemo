...
- 当前场景渲染流程关闭 SkyCube Pass，保留不透明物体与后处理通路。
- 调整颜色后处理的局部准备顺序；工程补丁版本由 `1.3.0` 提升至 `1.3.1`。

## 2026-09-22 版本 1.3：灯光设置持久化

- `.lscene` 新增可选 `lighting` 节点，保存并恢复环境光、方向光和点光设置；旧场景缺少该节点时使用默认灯光。
- Lighting 面板新增 `Ambient Intensity` 控件，环境光颜色按该强度上传到 Light CBuffer。
- 新建场景重置默认灯光；补充场景灯光往返和 LightManager 恢复测试，VS2022 Debug 构建及 5 项测试通过。
- 本次新增灯光编辑与持久化功能，工程版本由 `1.2.0` 提升至 `1.3.0`。
