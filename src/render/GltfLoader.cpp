...
MeshPart LoadPrimitive(const cgltf_data& data, const cgltf_node& node, const cgltf_primitive& primitive,
                       const std::filesystem::path& modelPath, ResourceCache& resources)
{
    ...
    for (cgltf_size index = 0; index < positions->count; ++index)
    {
        ...
        normal.z = -normal.z;
        vertices[index] = {position, normal, Vector2{textureValues[0], textureValues[1]}};
    }

    const std::size_t indexCount = primitive.indices != nullptr ? primitive.indices->count : positions->count;
...
std::shared_ptr<MeshAsset> GltfLoader::Import(const std::filesystem::path& path, ResourceCache& resources) const
{
    ...
    {
        throw std::runtime_error("Failed to parse glTF " + pathText + ": " + ResultName(result));
    }
    std::unique_ptr<cgltf_data, CgltfDeleter> data(rawData);
    result = cgltf_load_buffers(&options, data.get(), pathText.c_str());
    if (result != cgltf_result_success)
    {
        throw std::runtime_error("Failed to load glTF buffers " + pathText + ": " + ResultName(result));
    }
    result = cgltf_validate(data.get());
    if (result != cgltf_result_success)
    {
        throw std::runtime_error("glTF validation failed " + pathText + ": " + ResultName(result));
    }
