...
public:
    ...
    [[nodiscard]] std::shared_ptr<Texture2D> CreateTexture(
        std::string name, const std::filesystem::path& path, bool forceSrgb = true);
    [[nodiscard]] std::shared_ptr<Texture2D> AddTexture(
    ...
    [[nodiscard]] ID3D11ShaderResourceView* GetShaderResource(
...
protected:
    ...
    [[nodiscard]] ID3D11DeviceContext* DeviceContext() const noexcept { return m_context; }
    [[nodiscard]] EffectResource& Resources() noexcept { return m_resources; }
    [[nodiscard]] const EffectResource& Resources() const noexcept { return m_resources; }
