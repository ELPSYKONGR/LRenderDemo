...
- **原因**：保留 glTF 数据流的学习可见性，同时避免自行维护格式解析器；C 接口也能验证工程的 C/C++
  混合编译边界。
- **结果**：`core/Scene` 只保存模型路径，DX11 对象集中在 `render/`；模型、纹理与 Sampler 由
  `ResourceCache` 复用。当前只承诺静态三角网格和 BaseColor，不把近似高光称为完整 PBR。
- **状态**：已接受。
...
- **备选方案**：让 `Entity` 直接持有 SRV/Sampler，修改缓存中的 `ModelPart::material`，或保存与 API
  无关的实体材质参数并在绘制时合并。
