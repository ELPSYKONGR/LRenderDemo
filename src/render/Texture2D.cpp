...
{
    constexpr std::array<std::byte, 4> magic{std::byte{'D'}, std::byte{'D'}, std::byte{'S'}, std::byte{' '}};
    return bytes.size() >= magic.size() && std::equal(magic.begin(), magic.end(), bytes.begin());
}

} // namespace

Texture2D::Texture2D(Microsoft::WRL::ComPtr<ID3D11Resource> resource,
...
std::shared_ptr<Texture2D> Texture2D::LoadFile(ID3D11Device* device, ID3D11DeviceContext* context,
                                               const std::filesystem::path& path, bool forceSrgb)
{
    ...
    if (extension == L".dds")
    {
        ...
        result = DirectX::CreateDDSTextureFromFileEx(device, context, path.c_str(), 0, D3D11_USAGE_DEFAULT,
                                                     D3D11_BIND_SHADER_RESOURCE, 0, 0, flags, resource.GetAddressOf(),
                                                     view.GetAddressOf());
    }
    else
    {
        const auto flags = forceSrgb ? DirectX::WIC_LOADER_FORCE_SRGB : DirectX::WIC_LOADER_DEFAULT;
        result = DirectX::CreateWICTextureFromFileEx(device, context, path.c_str(), 0, D3D11_USAGE_DEFAULT,
                                                     D3D11_BIND_SHADER_RESOURCE | D3D11_BIND_RENDER_TARGET, 0,
                                                     D3D11_RESOURCE_MISC_GENERATE_MIPS, flags, resource.GetAddressOf(),
                                                     view.GetAddressOf());
    }
    if (FAILED(result))
    {
    ...
    return std::shared_ptr<Texture2D>(new Texture2D(std::move(resource), std::move(view), PathForMessage(path)));
}

std::shared_ptr<Texture2D> Texture2D::LoadMemory(ID3D11Device* device, ID3D11DeviceContext* context,
                                                 ...
                                                 bool forceSrgb)
{
    ...
    if (IsDds(bytes))
    {
        ...
        result = DirectX::CreateDDSTextureFromMemoryEx(
            device, context, reinterpret_cast<const std::uint8_t*>(bytes.data()), bytes.size(), 0, D3D11_USAGE_DEFAULT,
            D3D11_BIND_SHADER_RESOURCE, 0, 0, flags, resource.GetAddressOf(), view.GetAddressOf());
    }
    else
    {
        const auto flags = forceSrgb ? DirectX::WIC_LOADER_FORCE_SRGB : DirectX::WIC_LOADER_DEFAULT;
        result = DirectX::CreateWICTextureFromMemoryEx(
            device, context, reinterpret_cast<const std::uint8_t*>(bytes.data()), bytes.size(), 0, D3D11_USAGE_DEFAULT,
            D3D11_BIND_SHADER_RESOURCE | D3D11_BIND_RENDER_TARGET, 0, D3D11_RESOURCE_MISC_GENERATE_MIPS, flags,
    ...
    return std::shared_ptr<Texture2D>(new Texture2D(std::move(resource), std::move(view), std::move(sourceName)));
}
...
std::shared_ptr<Texture2D> Texture2D::CreateChecker(ID3D11Device* device)
{
    ...
    D3D11_SUBRESOURCE_DATA initialData{pixels.data(), textureSize * sizeof(std::uint32_t), 0};
    ...
    return std::shared_ptr<Texture2D>(new Texture2D(std::move(resource), std::move(view), "generated://checker"));
}
...
std::shared_ptr<Texture2D> Texture2D::CreateSolidWhite(ID3D11Device* device)
{
    ...
    return std::shared_ptr<Texture2D>(new Texture2D(std::move(resource), std::move(view), "generated://white"));
}

ID3D11ShaderResourceView* Texture2D::ShaderResourceView() const noexcept
{
    return m_shaderResourceView.Get();
}

const std::string& Texture2D::SourceName() const noexcept
{
    return m_sourceName;
}
