...
ViewManager::ViewManager(ID3D11Device* device, ID3D11DeviceContext* context)
...
ViewId ViewManager::CreateView(
    std::uint32_t width, std::uint32_t height, HWND windowHandle) {
    ViewInfo view;
...
void ViewManager::SetDepthMode(DepthMode mode) {
    ...
    description.DepthWriteMask = mode == DepthMode::ReadWrite
...
void ViewManager::SetStencil(const StencilDescription& stencil) {
    ...
    description.FrontFace = {
        stencil.failOperation, stencil.depthFailOperation, stencil.passOperation,
        stencil.comparison};
