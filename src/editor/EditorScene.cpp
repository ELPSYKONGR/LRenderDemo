...
std::filesystem::path SelectSceneFile(HWND owner, bool save, const std::filesystem::path& currentPath)
{
    ...
    if (error != 0)
    {
        throw std::runtime_error("Native scene file dialog failed with code " + std::to_string(error));
    }
    return {};
}
...
void EditorLayer::DrawSceneFileMenu(Scene& scene, CommandHistory& history, Dx11Renderer& renderer)
{
    if (m_exitRequested)
    {
...
bool EditorLayer::SaveScene(const Scene& scene, CommandHistory& history, Dx11Renderer& renderer, bool saveAs)
{
    try
    {
...
void EditorLayer::QueueSceneAction(int action, Scene& scene, CommandHistory& history, Dx11Renderer& renderer,
...
void EditorLayer::ExecuteSceneAction(Scene& scene, CommandHistory& history, Dx11Renderer& renderer)
{
...
void EditorLayer::DrawSceneFilePopups(Scene& scene, CommandHistory& history, Dx11Renderer& renderer)
{
