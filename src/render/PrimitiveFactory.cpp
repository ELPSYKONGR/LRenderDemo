...
void AddFace(std::vector<Vertex>& vertices, std::vector<std::uint32_t>& indices,
             const std::array<DirectX::XMFLOAT3, 4>& positions, const DirectX::XMFLOAT3& normal)
{
    const auto base = static_cast<std::uint32_t>(vertices.size());
    constexpr std::array<DirectX::XMFLOAT2, 4> textureCoordinates{
        DirectX::XMFLOAT2{0.0F, 1.0F}, DirectX::XMFLOAT2{1.0F, 1.0F}, DirectX::XMFLOAT2{1.0F, 0.0F},
        DirectX::XMFLOAT2{0.0F, 0.0F}};
    for (std::size_t index = 0; index < positions.size(); ++index)
    {
        vertices.push_back({positions[index], normal, textureCoordinates[index]});
    }
    const std::array<std::uint32_t, 6> faceIndices{base, base + 1, base + 2, base, base + 2, base + 3};
    indices.insert(indices.end(), faceIndices.begin(), faceIndices.end());
}

} // namespace

std::unique_ptr<Mesh> PrimitiveFactory::Create(ID3D11Device* device, const SolidGeometry& geometry)
{
    return std::visit(
...
std::unique_ptr<Mesh> PrimitiveFactory::CreateCube(ID3D11Device* device, const CubeParameters& parameters)
{
...
std::unique_ptr<Mesh> PrimitiveFactory::CreateSphere(ID3D11Device* device, const SphereParameters& parameters)
{
    ...
    const std::size_t vertexCount = (static_cast<std::size_t>(slices) + 1) * (static_cast<std::size_t>(stacks) + 1);
    ...
    for (std::uint32_t stack = 0; stack <= stacks; ++stack)
    {
        ...
        for (std::uint32_t slice = 0; slice <= slices; ++slice)
        {
            ...
            const DirectX::XMFLOAT3 normal{radius * std::sin(longitude), y, radius * std::cos(longitude)};
            const DirectX::XMFLOAT3 position{normal.x * parameters.radius, normal.y * parameters.radius,
                                             normal.z * parameters.radius};
            const DirectX::XMFLOAT2 textureCoordinate{static_cast<float>(slice) / slices,
...
std::unique_ptr<Mesh> PrimitiveFactory::CreatePlane(ID3D11Device* device, const PlaneParameters& parameters)
{
    const std::uint32_t columns = static_cast<std::uint32_t>(parameters.subdivisionsZ) + 1U;
    const std::size_t vertexCount = (static_cast<std::size_t>(parameters.subdivisionsX) + 1U) * columns;
    if (vertexCount > std::numeric_limits<std::uint32_t>::max())
    {
    ...
    std::vector<std::uint32_t> indices;
    vertices.reserve(vertexCount);
    indices.reserve(static_cast<std::size_t>(parameters.subdivisionsX) * parameters.subdivisionsZ * 6U);
    ...
    for (std::uint32_t x = 0; x <= parameters.subdivisionsX; ++x)
    {
        ...
        for (std::uint32_t z = 0; z <= parameters.subdivisionsZ; ++z)
        {
            ...
            vertices.push_back({{-halfX + parameters.size.x * tx, 0.0F, -halfZ + parameters.size.y * tz},
                                {0.0F, 1.0F, 0.0F},
    ...
    for (std::uint32_t x = 0; x < parameters.subdivisionsX; ++x)
    {
        for (std::uint32_t z = 0; z < parameters.subdivisionsZ; ++z)
        {
            ...
            indices.insert(indices.end(), {first, first + 1U, next + 1U, first, next + 1U, next});
        }
    }
    return std::make_unique<Mesh>(device, vertices, indices);
}
